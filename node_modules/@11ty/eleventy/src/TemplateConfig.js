...
class TemplateConfig {
  constructor(customRootConfig, projectConfigPath) {
    ...
    /**
     * @member {module:11ty/eleventy/TemplateConfig~TemplateConfig~override} -
     * tbd.
     */
    this.overrides = {};
  ...
  getLocalProjectConfigFile() {
    ...
    // Add the configFiles[0] in case of a test, where no file exists on the
    // file system
  ...
  /**
   * Gets the current path prefix denoting the root folder the output will be
   * deployed to
   *
  ...
  initializeRootConfig() {
    ...
    if (typeof this.rootConfig === 'function') {
      ...
      // debug( "rootConfig is a function, after calling, this.userConfig is
      // %o", this.userConfig );
    }
   ...
   *
   * @param {Object} - a subset of the return Object from the user’s config
   *     file.
   */
  ...
  processPlugins({dir, pathPrefix}) {
    ...
    for (let {plugin, options, pluginNamespace} of this.userConfig.plugins) {
      try {
        ...
        let namespaces =
            [storedActiveNamespace, pluginNamespace].filter((entry) => !!entry);
        ...
        throw new EleventyPluginError(
            `Error processing ${name ? `the \`${name}\`` : 'a'} plugin${
                namespaceStr}`,
            e);
      }
    }
  ...
  requireLocalConfigFile() {
    let localConfig = {};
    let path = this.projectConfigPaths.filter((path) => path)
    ...
    if (path) {
      try {
        ...
        if (typeof localConfig === 'function') {
          ...
          // debug( "localConfig is a function, after calling, this.userConfig
          // is %o", this.userConfig );

          if (typeof localConfig === 'object' &&
              typeof localConfig.then === 'function') {
            throw new EleventyConfigError(
                `Error in your Eleventy config file '${
                    path}': Returning a promise is not yet supported.`);
          }
        }

        // Still using removed `filters`? this was renamed to transforms
        if (localConfig && localConfig.filters !== undefined &&
        ...
        // TODO the error message here is bad and I feel bad (needs more
        // accurate info)
        throw new EleventyConfigError(
  ...
  mergeConfig() {
    ...
    // Returning a falsy value (e.g. "") from user config should reset to the
    // default value.
    if (!mergedConfig.pathPrefix) {
      mergedConfig.pathPrefix = this.rootConfig.pathPrefix;
    }

    // Delay processing plugins until after the result of localConfig is
    // returned But BEFORE the rest of the config options are merged this way we
    // can pass directories and other template information to plugins
    ...
    if (eleventyConfigApiMergingObject &&
